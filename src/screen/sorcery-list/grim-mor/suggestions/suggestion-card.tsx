...
import { Button, Card, Classes, H1, H2 } from '@blueprintjs/core';
...
const SuggestionCard = ({ entity }: Props) => {
  ...
  return (
    <Card className={classNames('suggestion-card-root')} onClick={handleClick}>
      <div>
        <H1>{entity}</H1>
        <H2>
      ...
      <Button className={classNames(Classes.OUTLINED)}>
        {t('sorcery-list:grimMor.template.select')}
      </Button>
    </Card>
  );
};
